...
void dynamicArrayTest()
{
    ...
    for (size_t i = 1; i <= 2000; ++i)
    {
        ...
        if (err)
        {
            printf("Function pushDa(): Error No: %d, while pushing element at index: %d\n.", err, i);
            return;
        }
    }
    puts("Function pushDA() test: success! ");

    int *output = NULL;
    for (size_t i = 1; i <= 2000; ++i)
    {
        int err = atDA(da, i - 1, (void **)&output);
        if (err)
        {
            printf("Function atDA(): Error No: %d, while accessing element at index: %d\n.", err, i);
            return;
        }
        if (i != *output)
        {
            printf("Function atDA(): Incorrect element value (%d) at index (%d).\n", *output, i);
            return;
        }
        // printf("i: %d, output: %d, err: %d\n", i, *output, err);
    }
    puts("Function atDA() test: success! ");
    ...
    puts("Function unshiftDA() test: success! ");

    for (size_t i = 0; i < da->count; ++i)
    {
        int err = atDA(da, i, (void **)&output);
        if (err)
        {
            printf("Function atDA(): Error No: %d, while accessing element at index: %d\n.", err, i);
            return;
    ...
    if (da->count != 2010)
    {
        printf("Expected: 2020, received: %d\n", da->count);
    }
    else
    {
        printf("Correct.\n");
    ...
    int *outInt = (int *)malloc(sizeof(int));
    for (size_t i = 9; i >= 0; --i)
    {
        int err = shiftInt(da, outInt);
        if (err)
        {
            printf("Function shiftInt(): Error No: %d, while accessing element at index: %d\n.", err, i);
            return;
        }
        else if (*outInt != i)
        {
            printf("Function shiftInt(): expected: %d, received: %d\n.", i, *outInt);
            free(outInt);
            return;
        }
    }
    free(outInt);
    puts("Function shiftInt() test: success! ");

    freeDynamicArray(da);
    puts("Freeing DynamicArray...");
    puts("Test done successfully.");
    puts("#############################");
}
*/
